...
Impact: Launcher/icon binaries remain excluded from source changes; wrapper binaries are handled by dedicated wrapper decision records.
...
Impact: Replaced by DEC-011 because this repository requires standard committed wrapper files for CI reliability.


## [DEC-011] Restore Standard Committed Gradle Wrapper
Date: 2026-03-04
Status: Superseded

Decision: Track full Gradle wrapper artifacts (`gradlew`, `gradlew.bat`, `gradle/wrapper/gradle-wrapper.jar`, `gradle/wrapper/gradle-wrapper.properties`) and use wrapper build in CI (`./gradlew :app:assembleDebug --stacktrace`).
Reason: Current failure is missing wrapper JAR; wrapper must be committed for deterministic CI bootstrap.
Impact: Superseded by DEC-012 for Codex-compatible binary-free PR updates.

## [DEC-012] Codex-Compatible Binary-Free Gradle Wrapper
Date: 2026-03-05
Status: Decided

Decision: Omit `gradle/wrapper/gradle-wrapper.jar` from git while keeping `gradlew`, `gradlew.bat`, and `gradle/wrapper/gradle-wrapper.properties` pinned to Gradle 9.4.0.
Reason: Binary files are not supported by Codex PR update flow; committing wrapper JAR blocks PR updates outside Codex.
Impact: Gradle wrapper JAR omitted due to Codex binary limitation. CI reliably bootstraps via `gradle/actions/setup-gradle@v5`. Local devs can run `gradle wrapper` once if they want the full JAR.
